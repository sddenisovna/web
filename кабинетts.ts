...
function updateProfileUI(): void {
    ...
    if (profileDateEl) profileDateEl.textContent = currentUser.registeredAt;
}
...
function initClearSubscriptions(): void {
...
function initTabs(): void {
    ...
    function showTab(tabId: string): void {
        ...
        if (trainingsTab) trainingsTab.style.display = 'none';
        if (subscriptionsTab) subscriptionsTab.style.display = 'none';
...
(window as any).addTraining = function(training: Training): boolean {
    const userId = getCurrentUserId();
    if (!userId) {
...
(window as any).addSubscription = function(subscription: Subscription): boolean {
    const userId = getCurrentUserId();
    if (!userId) {
        showNotification('ошибка: пользователь не найден', true);
        return false;
