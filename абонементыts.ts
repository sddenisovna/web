...
function saveToCabinet(subName: string): void {
...
(window as any).selectSubscription = function(subName: string) {
    ...
    if (userId) {
        ...
        (window as any).pendingSubscription = subName;
...
function initButtons(): void {
    ...
    btns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (subName) {
                (window as any).selectSubscription(subName);
            }
        });
    });
}
...
function initModal(): void {
    ...
    if (loginSubmit) {
        loginSubmit.onclick = () => {
            ...
            const pending = (window as any).pendingSubscription;
            if (pending) {
                saveToCabinet(pending);
                (window as any).pendingSubscription = null;
            }
