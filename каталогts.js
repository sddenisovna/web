...
function addTrainingToCabinet(training) {
  ...
  const exists = trainings.some((t) => t.id === training.id);
  if (!exists) {
...
function renderTrainings() {
  ...
  const sport = getElement("sportFilter")?.value || "";
  const district = getElement("districtFilter")?.value || "";
  const format = getElement("formatFilter")?.value || "";
  const level = getElement("levelFilter")?.value || "";
  const filtered = trainingsData.filter((t) => {
    if (searchTerm && !t.title.toLowerCase().includes(searchTerm) && !t.trainer.toLowerCase().includes(searchTerm))
      return false;
    ...
    if (format && t.format !== format)
      return false;
    if (level && level !== "любой" && t.level !== level)
      return false;
    return true;
  });
...
function resetFilters() {
  ...
  const sportFilter = getElement("sportFilter");
  const districtFilter = getElement("districtFilter");
  const dateFilter = getElement("dateFilter");
  const timeFilter = getElement("timeFilter");
  const formatFilter = getElement("formatFilter");
  const levelFilter = getElement("levelFilter");
  if (searchInput)
    searchInput.value = "";
  if (sportFilter)
    sportFilter.value = "";
